...
void main() {
  ...
  group('JourneyNotifier & JourneyProvider Tests', () {
    test(
      'Initial Journey state starts at Level 1 with 0 solved count',
      () async {
    ...
    test(
      'completeCurrentLevel increments level and solved count and unlocks milestone',
      () async {
        final container = ProviderContainer();
        addTearDown(container.dispose);
    ...
    test(
      'getPuzzleForLevel correctly returns mapped puzzle from sorted list',
      () {
        final container = ProviderContainer();
        addTearDown(container.dispose);
