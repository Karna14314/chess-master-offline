...
void main() {
  group('Difficulty System Tests', () {
    ...
    test(
      'setSkillLevel sends only UCI_Elo (no Skill Level conflict)',
      () async {
        service.forceFallback = true;
        await service.initialize();
    ...
    test(
      'engine_provider configures strength once per game (not per move)',
      () async {
    ...
    test(
      'engine_provider honors Simple Bot mode without Stockfish init',
      () async {
    ...
    test('all difficulty parameters are internally consistent', () {
      for (final level in AppConstants.difficultyLevels) {
        ...
        expect(level.level, AppConstants.difficultyLevels.indexOf(level) + 1);
      }
    });
  });
}
